...
class Luxury{
	...
	Luxury(String username,long contactNumber,double uberAccBalance, int pkm,String drop,double price){
	...
	public boolean bookCab() {
		if(super.login()) {
			double total = uberAccBalance - price;
			if(total<=0) {
				return false;
			}
			return true;
	...
	public void display() {
		if(bookCab()) {
			System.out.println("Your Luxury cab has been booked with price: "+price);
